    ...
    <div style={{ position: "relative" }}>
      <div className="cc" style={{ background: `linear-gradient(135deg,${card.grad[0]},${card.grad[1]})` }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", position: "relative", marginBottom: 22 }}>
          <div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.8)", marginBottom: 3, fontWeight: 600, letterSpacing: "0.5px" }}>
              {card.flag}
            </div>
            <div style={{ fontSize: 15, fontWeight: 800, color: "#fff", paddingRight: 70 }}>
              {card.name}
            </div>
          </div>
          <div style={{ fontSize: 24, color: "rgba(255,255,255,0.6)", position: "absolute", bottom: 0, right: 0 }}>▣</div>
        </div>

        {/* Número mascarado */}
        <div style={{ fontSize: 15, fontWeight: 600, letterSpacing: "3px", marginBottom: 16, color: "rgba(255,255,255,0.9)" }}>
          •••• •••• •••• {card.digits}
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.7)", marginBottom: 2, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase" }}>
              Fatura Atual
            </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 5, fontSize: 10, color: "rgba(255,255,255,0.65)" }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 7, alignItems: "center" }}>
        ...
        <span style={{ fontWeight: 800, color: warn ? "var(--red)" : "var(--green)", flexShrink: 0 }}>
          {pct.toFixed(0)}%
        </span>
      </div>
...
export default function Cartoes({ cards, openNewCard, openEditCard, removeCard }) {
  // Totalizadores
  const totalFatura     = cards.reduce((sum, c) => sum + c.balance, 0);
  const totalLimite     = cards.reduce((sum, c) => sum + c.limit, 0);
  const totalDisponivel = cards.reduce((sum, c) => sum + Math.max(0, c.limit - c.balance), 0);
  ...
  return (
    <>
      ...
      <div className="pg-sub">
        Controle de faturas e limites · {cards.length} cartão{cards.length !== 1 ? "s" : ""} cadastrado{cards.length !== 1 ? "s" : ""}
      </div>

      {/* ── Empty state ── */}
      {cards.length === 0 && (
        <div style={{
          background: "var(--surface)", border: "1px dashed rgba(232,184,109,0.25)",
          ...
          <button
            ...
            style={{
              ...
              background: "var(--gold3)", border: "1px solid rgba(232,184,109,0.3)",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
                ...
                <div style={{ flexShrink: 0, position: "relative", width: 160, height: 160 }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: 9, fontWeight: 700, color: "var(--text2)", letterSpacing: "1px", textTransform: "uppercase" }}>
                      Total
                    </div>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <div style={{
                              ...
                              background: card.grad[0], flexShrink: 0,
                            }} />
                            <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>
                              {card.name}
                            </span>
                          </div>
                          <span style={{ fontSize: 13, fontWeight: 800, color: card.grad[0] }}>
                            {sharePct}%
                          </span>
                        </div>
