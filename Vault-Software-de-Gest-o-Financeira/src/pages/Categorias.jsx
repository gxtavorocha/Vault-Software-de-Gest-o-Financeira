...
export default function Categorias({ categories, catForm, setCatForm, addCat, removeCat }) {
  return (
    <>
      ...
      <div className="cp">
        ...
        <div className="panel">
          ...
          <div className="field">
            ...
            <input className="finp" placeholder="Ex: Pets, Investimentos..." value={catForm.label}
          ...
          <div className="field">
            ...
            <div className="ig">
              {PRESET_ICONS.map(ic => (
                <div key={ic} className={`io${catForm.icon === ic ? " sel" : ""}`} onClick={() => setCatForm(f => ({ ...f, icon: ic }))}>{ic}</div>
              ))}
            </div>
          </div>
          ...
          <div className="field">
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {PRESET_COLORS.map(col => (
                <div key={col} className={`cdot2${catForm.color === col ? " sel" : ""}`} style={{ background: col }} onClick={() => setCatForm(f => ({ ...f, color: col }))} />
              ))}
            </div>
