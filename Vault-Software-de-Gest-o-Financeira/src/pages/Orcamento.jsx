...
export default function Orcamento({
  month, year, totalIncome, balance,
  activePlanId, setActivePlanId, activePlan,
  customPlans, removePlan,
  budgetGroups, customRows,
  customBudget, updCustPct, customTotal,
  budTab, setBudTab,
  categories, setShowPlanModal,
  toast$,
}) {
  return (
    <>
      ...
      <div className="pg-sub">
        {MONTHS[month]} {year} · Plano: <strong style={{ color: "var(--gold)" }}>{activePlan?.name}</strong>
        {" "}· Renda confirmada: <strong style={{ color: "var(--green)" }}>{fmt(totalIncome)}</strong>
      </div>
      ...
      <div className="btabs">
        ...
        {activePlanId === "custom" && (
          <button className={`btab${budTab === "custom" ? " on" : ""}`} onClick={() => setBudTab("custom")}>⚙ Configurar</button>
        )}
      </div>
      ...
      {budTab === "planos" && (
        <>
          <div style={{ fontSize: 11, color: "var(--gold)", fontWeight: 600, letterSpacing: "0.5px", marginBottom: 12 }}>PLANOS PREDEFINIDOS</div>
          <div className="pgrid">
                ...
                <div className="pgrow">
                  {plan.groups.map(g => (
                    <div key={g.id} className="pgr">
                      <span style={{ fontSize: 10, color: g.color, fontWeight: 700, width: 54, flexShrink: 0 }}>{g.label.slice(0, 9)}</span>
                  ...
                  <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    {plan.groups.map(g => (
                      <div key={g.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3 }}>
                        ...
                        <span style={{ color: "var(--text)", fontWeight: 600 }}>{fmt((g.pct / 100) * totalIncome)}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className={`pcard${activePlanId === "custom" ? " active" : ""}`}
            ...
            <div style={{ fontSize: 28 }}><GrConfigure /></div>
            <div>
                  ...
                  <div key={plan.id} className={`pcard${activePlanId === plan.id ? " active" : ""}`}>
                    <div style={{ position: "absolute", top: 14, right: 14, display: "flex", gap: 6, zIndex: 1 }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); removePlan(plan.id); }}
                        style={{ background: "rgba(232,122,109,0.1)", border: "1px solid rgba(232,122,109,0.25)", color: "var(--red)", cursor: "pointer", fontSize: 11, padding: "2px 8px", borderRadius: 8, fontWeight: 700, fontFamily: "var(--font)" }}>✕</button>
                    </div>
                      ...
                      <div className="pgrow">
                        {(plan.groups || []).map((g, i) => (
                          <div key={i} className="pgr">
                            <span style={{ fontSize: 10, color: g.color || "var(--gold)", fontWeight: 700, width: 54, flexShrink: 0 }}>{(g.label || "").slice(0, 9)}</span>
          ...
          <button onClick={() => setShowPlanModal(true)} style={{
            marginTop: 6, padding: "11px 24px", borderRadius: 12,
            border: "1px dashed rgba(232,184,109,0.3)", background: "rgba(232,184,109,0.05)",
                ...
                <div key={r.id} className="bg-item">
                  <div className="bg-hd">
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 34, height: 34, borderRadius: 10, background: r.color + "28", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17 }}>{r.icon}</div>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700 }}>{r.label}</div>
                        {/* ← cor da categoria aqui */}
                        <div style={{ fontSize: 11, color: r.color, opacity: 0.85, fontWeight: 600, marginTop: 2 }}>
                          {fmtPct(r.allocPct)} da renda · {fmtPct(r.pctOfInc)} utilizado
                        </div>
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 18, fontWeight: 800, fontFamily: "var(--display)", color: over ? "var(--red)" : r.color }}>{fmt(r.spent)}</div>
                      <div style={{ fontSize: 11, color: r.color, opacity: 0.7, fontWeight: 600 }}>de {fmt(r.limit)}</div>
                    </div>
                  </div>
                  <div className="bg-track">
                    <div className="bg-fill" style={{ width: `${r.usedPct}%`, background: over ? "var(--red)" : r.color, boxShadow: over ? "0 0 8px rgba(232,122,109,0.4)" : `0 0 10px ${r.color}88` }} />
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
                    <span style={{ color: over ? "var(--red)" : "var(--green)", fontWeight: 700 }}>
                      {over ? `⚠️ ${fmt(r.limit - r.spent)} restante` : `✓ ${fmt(r.limit - r.spent)} disponível`}
                    </span>
                    {/* ← cor da categoria aqui */}
                    <span style={{ color: r.color, opacity: 0.75, fontWeight: 600 }}>{fmtPct(r.usedPct)} do limite</span>
                  </div>
                </div>
              );
            })}
            ...
            <div className="g3">
              {budgetGroups.map(g => (
                <div key={g.id} className="sc">
                  ...
                  <div className="sc-sub" style={{ color: g.color, opacity: 0.7, fontWeight: 600 }}>meta: {g.pct}% = {fmt(g.limit)}</div>
                  <div style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: g.id === "pou" ? (balance >= (g.pct / 100) * totalIncome ? "var(--green)" : "var(--gold)") : (g.spent > g.limit ? "var(--red)" : "var(--green)") }}>
                    {g.id === "pou" ? fmtPct(totalIncome > 0 ? Math.max(0, balance) / totalIncome * 100 : 0) + " poupado" : fmtPct(totalIncome > 0 ? g.spent / totalIncome * 100 : 0) + " da renda"}
                  </div>
                </div>
              ))}
            </div>
              ...
              <div key={g.id} className="bg-item">
                <div className="bg-hd">
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: g.color + "28", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{g.icon}</div>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700 }}>{g.label}</div>
                      {/* ← cor do grupo aqui */}
                      <div style={{ fontSize: 11, color: g.color, opacity: 0.8, fontWeight: 600, marginTop: 2 }}>
                        Meta: {fmtPct(g.pct)} da renda = {fmt(g.limit)}
                      </div>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 20, fontWeight: 800, fontFamily: "var(--display)", color: g.color }}>{g.id === "pou" ? fmt(Math.max(0, balance)) : fmt(g.spent)}</div>
                    {/* ← cor do grupo aqui */}
                    <div style={{ fontSize: 11, color: g.color, opacity: 0.7, fontWeight: 600 }}>
                      {g.id === "pou" ? (totalIncome > 0 ? fmtPct(Math.max(0, balance) / totalIncome * 100) + " poupado" : "—") : (totalIncome > 0 ? fmtPct(g.spent / totalIncome * 100) + " da renda" : "—")}
                    </div>
                  </div>
                </div>
                <div style={{ height: 10, borderRadius: 99, background: "var(--surface3)", position: "relative", overflow: "hidden", margin: "12px 0 6px" }}>
                  ...
                  {g.id === "pou" ? (
                    <div style={{ position: "absolute", height: "100%", width: `${totalIncome > 0 ? Math.min(Math.max(0, balance) / totalIncome * 100, 100) : 0}%`, background: balance >= (g.pct / 100) * totalIncome ? "var(--green)" : "var(--gold)", borderRadius: 99, transition: "width 1s", boxShadow: "0 0 10px rgba(109,232,160,0.5)" }} />
                  ) : (
                    <div style={{ position: "absolute", height: "100%", width: `${Math.min(g.usedPct, 100)}%`, background: g.spent > g.limit ? "var(--red)" : g.color, borderRadius: 99, transition: "width 1s", boxShadow: `0 0 10px ${g.color}88` }} />
                  )}
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: g.cats.length > 0 ? 12 : 0 }}>
                  {g.id === "pou" ? (
                    <span style={{ color: balance >= (g.pct / 100) * totalIncome ? "var(--green)" : "var(--gold)", fontWeight: 700 }}>
                      {balance >= (g.pct / 100) * totalIncome ? "✓ Meta atingida!" : `Faltam ${fmt(Math.max(0, (g.pct / 100) * totalIncome - balance))} para a meta`}
                    </span>
                  ) : (
                    <span style={{ color: g.spent > g.limit ? "var(--red)" : "var(--green)", fontWeight: 700 }}>
                      {g.spent > g.limit ? `⚠️ ${fmt(g.spent - g.limit)} acima do limite` : `✓ ${fmt(g.limit - g.spent)} disponível`}
                    </span>
                  )}
                  {/* ← cor do grupo aqui */}
                  <span style={{ color: g.color, opacity: 0.75, fontWeight: 600 }}>
                    {fmtPct(g.id === "pou" ? (totalIncome > 0 ? Math.min(Math.max(0, balance) / totalIncome * 100, 100) : 0) : g.usedPct)} do limite
                  </span>
                </div>
                {g.cats.map(cat => cat && (
                  <div key={cat.id} className="bg-cat">
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: (cat.color || "#888") + "28", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, flexShrink: 0 }}>{cat.icon}</div>
                    <div style={{ flex: 1, marginLeft: 4 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 4 }}>
                        <span style={{ fontWeight: 600, color: "var(--text)" }}>{cat.label}</span>
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          {/* ← cor da categoria aqui */}
                          <span style={{ fontSize: 10, color: cat.color || g.color, opacity: 0.8, fontWeight: 700 }}>{fmtPct(cat.pctOfInc)} da renda</span>
                          <span style={{ fontWeight: 800, color: cat.color || "var(--gold)" }}>{fmt(cat.spent)}</span>
                        </div>
                      </div>
                      <div style={{ height: 4, borderRadius: 99, background: "var(--surface3)", overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${Math.min(cat.pctOfGroup, 100)}%`, borderRadius: 99, background: cat.color || g.color, transition: "width 0.8s", boxShadow: `0 0 6px ${cat.color || g.color}88` }} />
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {budTab === "custom" && activePlanId === "custom" && (
        <div className="panel">
          <div className="ph">
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: Math.abs(customTotal - 100) < 0.5 ? "var(--green)" : customTotal > 100 ? "var(--red)" : "var(--gold)" }}>
                {customTotal.toFixed(1)}%
              </div>
              <div style={{ fontSize: 11, color: "var(--text3)" }}>
                {Math.abs(customTotal - 100) < 0.5 ? "✓ Total OK" : `${(100 - customTotal).toFixed(1)}% restante`}
              </div>
          ...
          {categories.map(cat => {
            ...
            return (
              <div key={cat.id} className="cust-row">
                <div style={{ width: 34, height: 34, borderRadius: 10, background: cat.color + "28", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{cat.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{cat.label}</div>
                  {/* ← cor da categoria aqui */}
                  <div style={{ fontSize: 11, color: e.pct > 0 ? cat.color : "var(--text3)", opacity: e.pct > 0 ? 0.85 : 1, fontWeight: 600, marginTop: 1 }}>
                    {e.pct > 0 ? `= ${fmt((e.pct / 100) * totalIncome)} / mês` : "Sem alocação"}
                  </div>
                </div>
